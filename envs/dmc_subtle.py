...
def _modify_xml_element_size(xml_string,
                             element_name,
                             new_size_str,
                             element_type="geom"):
    ...
    if element is not None:
        ...
        raise ValueError(
            f"Element '{element_name}' of type '{element_type}' not found in XML."
        )
...
# --- Reacher ---
def reacher_subtle(time_limit=reacher._DEFAULT_TIME_LIMIT,
                   random=None,
...
# --- Finger ---
def finger_turn_subtle(time_limit=finger._DEFAULT_TIME_LIMIT,
                       random=None,
                       environment_kwargs=None):
    ...
    task = finger.Turn(target_radius=_DEFAULT_SIZE *
                       SCALES["finger_turn_subtle"],
                       random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(physics,
                               task,
                               time_limit=time_limit,
                               control_timestep=finger._CONTROL_TIMESTEP,
                               **environment_kwargs)


# --- Point-mass ---
def point_mass_subtle(time_limit=point_mass._DEFAULT_TIME_LIMIT,
                      random=None,
                      environment_kwargs=None):
    ...
    modified_xml = _modify_xml_element_size(
        xml_string, "target",
        str(_DEFAULT_TARGET_SIZE * SCALES["point_mass_subtle"]))
    modified_xml = _modify_xml_element_size(
        modified_xml, "pointmass",
        str(_DEFAULT_AGENT_SIZE * SCALES["point_mass_subtle"]))
...
# --- Ball-in-cup ---
def ball_in_cup_catch_subtle(time_limit=ball_in_cup._DEFAULT_TIME_LIMIT,
                             random=None,
                             environment_kwargs=None):
    ...
    if ball is not None:
        ball.set("size",
                 str(_DEFAULT_BALL_SIZE * SCALES["ball_in_cup_catch_subtle"]))
    else:
    ...
    if string_tendon is not None:
        string_tendon.set(
            "width",
            str(_DEFAULT_STRING_WIDTH * SCALES["ball_in_cup_catch_subtle"]))
    else:
...
def _get_cartpole_subtle_physics(random=None):
    ...
    if default_pole_geom is None:
        raise ValueError(
            "Could not find default geom for class 'pole' in the cartpole XML model."
        )
...
def cartpole_swingup_subtle(time_limit=cartpole._DEFAULT_TIME_LIMIT,
                            random=None,
                            environment_kwargs=None):
