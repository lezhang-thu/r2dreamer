...
class TransformerRSSM(nn.Module):
    ...
    def __init__(self, config, embed_size, act_dim):
        ...
        if self._use_deter_feat:
            ...
            self._feat_deter_dim = D
        if self._use_stoch_feat:
            self._feat_stoch_dim = int(feat_stoch_dim)
            if self._feat_stoch_dim < 1:
                raise AssertionError("feat_stoch_dim must be positive.")
            ...
            self._feat_stoch_dim = self.flat_stoch
    ...
    def _make_logit_head(self, prefix, inp_dim, layers, act_fn):
        head = nn.Sequential()
        for i in range(layers):
            head.add_module(f"{prefix}_{i}",
                            nn.Linear(inp_dim, self._head_hidden, bias=True))
            head.add_module(
                f"{prefix}_n_{i}",
                nn.RMSNorm(self._head_hidden, eps=1e-04, dtype=torch.float32))
            head.add_module(f"{prefix}_a_{i}", act_fn())
            inp_dim = self._head_hidden
        head.add_module(
            f"{prefix}_logit",
            nn.Linear(inp_dim, self._stoch * self._discrete, bias=True))
    ...
    def observe(self,
        ...
        # Pass 2: final posterior conditioned on the proposal context. Prior,
        # heads, and imagination starts use the final deterministic context
        # generated by this refined stochastic state.
        ...
        entries = {'deter': h2_prev, 'stoch': stoch}
        feat = {
            'deter': h2_prev,
    ...
    def prior(self, deter):
        ...
        logit = self._prior_head(self._deter_context(deter))
